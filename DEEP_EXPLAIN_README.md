...
===
DeepExplain provides a unified framework for state-of-the-art gradient _and_ perturbation-based attribution methods.
...
- [**Occlusion**](https://arxiv.org/abs/1311.2901), as an extension
  of the [grey-box method by Zeiler _et al_](https://arxiv.org/abs/1311.2901).
...
When the attributions of all input features are arranged together to have the same shape of the input sample we talk about _attribution maps_ (as in the picture below), where red and blue colors indicate respectively features that contribute positively to the activation of the target output and features having a suppressing effect on it.
...
**Softmax**: if the network last activation is a Softmax, it is recommanded to target the activations _before_ this normalization.

### Performance: Explainer API

If you need to run `explain()` multiple times (for example, new data to process with the same model comes in over time) it is recommanded that you use the Explainer API. This provides a way to _compile_ the graph operations needed to generate the explanations and _evaluate_ this graph in two different steps.
