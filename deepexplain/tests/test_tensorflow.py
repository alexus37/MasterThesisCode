...
from deepexplain.tensorflow.utils import original_grad  # test only
...
def min_model_2d(session):
    ...
    Implements min(xi)
    """
    X = tf.compat.v1.placeholder("float", [None, 2, 2])
...
def simple_multi_inputs_model(session):
    ...
    X2 = tf.compat.v1.placeholder("float", [None, 2])
...
def simple_multi_inputs_model2(session):
    ...
    """
    X1 = tf.compat.v1.placeholder("float", [None, 2])
    X2 = tf.compat.v1.placeholder("float", [None, 1])
...
def train_xor(session):
    ...
    w1 = tf.Variable(
        initial_value=[[0.10711301, -0.0987727], [-1.57625198, 1.34942603]])
    ...
    loss = tf.reduce_mean(tf.compat.v1.losses.mean_squared_error(Y, out))
...
class TestDeepExplainGeneralTF(TestCase):
    ...
    def test_context(self):
        ...
        for name in activations:
            ...
            self.assertEqual(x1_g.op.type,
                             'Mul' if name == 'Softplus' else f'{name}Grad')

        # Override (note: that need to pass graph! Multiple thread testing??)
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            for name in activations:
                # Gradients of nonlinear ops are overriden
                x2 = activations[name](X)
                self.assertEqual(x2.op.get_attr(
                    '_gradient_op_type').decode('utf-8'), 'DeepExplainGrad')

    def test_mismatch_input_lens(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            X1 = tf.compat.v1.placeholder("float", [None, 1])
            X2 = tf.compat.v1.placeholder("float", [None, 1])
    ...
    def test_supported_activations(self):
        ...
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            np.testing.assert_almost_equal(
                r[0], [0.268941421, 0.5, 0.731058578], 7)
            Y = tf.nn.tanh(X)
            r = self.session.run(Y, {X: xi})
            np.testing.assert_almost_equal(
                r[0], [-0.761594155, 0, 0.761594155], 7)
            Y = tf.nn.softplus(X)
            r = self.session.run(Y, {X: xi})
            np.testing.assert_almost_equal(
                r[0], [0.313261687, 0.693147181, 1.31326168], 7)
    ...
    def test_warning_unsupported_activations(self):
        with warnings.catch_warnings(record=True) as w:
            ...
            with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
                ...
                assert any(["unsupported activation" in str(wi.message)
                            for wi in w])
    ...
    def test_X_has_compatible_batch_dim(self):
        X = tf.compat.v1.placeholder("float", [10, 3])
        Y = tf.nn.relu(X)
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            with self.assertRaises(RuntimeError) as cm:
    ...
    def test_use_of_target_weights_batch(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            np.testing.assert_almost_equal(a1+a2, a3, 10)
            np.testing.assert_almost_equal(
                a4, np.array([[0.0, 0.0]]).repeat(20, 0), 10)
        ...
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            for i in range(10):
                ...
                if prev_ops_count is None:
                    ...
                    self.assertEqual(prev_ops_count, ops_count)
...
class TestSaliencyMethod(TestCase):
    ...
    def test_saliency_method(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, [[0.0, 0.0], [1.0, 1.0]], 10)
...
class TestGradInputMethod(TestCase):
    ...
    def test_saliency_method(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, [[0.0, 0.0], [3.0, -1.0]], 10)
...
class TestIntegratedGradientsMethod(TestCase):
    ...
    def test_int_grad(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, [[0.0, 0.0], [1.5, -0.5]], 10)
    ...
    def test_int_grad_baseline(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            xi = np.array([[2, 0]])
            attributions = de.explain(
                'intgrad', out, X, xi, baseline=np.array([1, 0]))
            self.assertEqual(attributions.shape, xi.shape)
    ...
    def test_int_grad_baseline_explainer_api(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            X, out = simpler_model(self.session)
            xi = np.array([[2, 0]])
            explainer = de.get_explainer(
                'intgrad', out, X, baseline=np.array([1, 0]))
            attributions = explainer.run(xi)
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(attributions, [[1.0, 0.0]], 10)
    ...
    def test_int_grad_baseline_2(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, [[1.0, 0.0], [2.0, 0.0]], 10)
...
class TestEpsilonLRPMethod(TestCase):
    ...
    def test_elrp_method(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, [[0.0, 0.0], [3.0, -1.0]], 3)
    ...
    def test_elrp_epsilon(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, [[0.0, 0.0], [3.0, -1.0]], 7)
...
class TestDeepLIFTMethod(TestCase):
    ...
    def test_deeplift(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, [[0.0, 0.0], [2.0, -1.0]], 10)
    ...
    def test_deeplift_batches(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(attributions, np.repeat(
                [[0.0, 0.0], [2.0, -1.0]], 25, 0), 10)
    ...
    def test_multiple_inputs_different_sizes_batches(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            X1, X2, out = simple_multi_inputs_model2(self.session)
            xi = [np.array([[-10, -5]]).repeat(50, 0),
                  np.array([[3]]).repeat(50, 0)]
            attributions = de.explain(
                'deeplift', out, [X1, X2], xi, batch_size=32)
            self.assertEqual(len(attributions), len(xi))
            np.testing.assert_almost_equal(
                attributions[0], np.repeat([[0.0, 0.0]], 50, 0), 10)
            np.testing.assert_almost_equal(
                attributions[1], np.repeat([[6]], 50, 0), 10)
    ...
    def test_multiple_inputs_different_sizes_batches_disable(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            xi = [np.array([[-10, -5]]).repeat(50, 0),
                  np.array([[3]]).repeat(50, 0)]
            attributions = de.explain(
                'deeplift', out, [X1, X2], xi, batch_size=None)
            self.assertEqual(len(attributions), len(xi))
            np.testing.assert_almost_equal(
                attributions[0], np.repeat([[0.0, 0.0]], 50, 0), 10)
            np.testing.assert_almost_equal(
...
class TestOcclusionMethod(TestCase):
    ...
    def test_window_shape(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            xi = np.array([[-10, -5], [3, 1]])
            attributions = de.explain(
                'occlusion', out, X, xi, window_shape=(2,))
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, [[0.0, 0.0], [0.5, 0.5]], 10)
    ...
    def test_nan_warning(self):
        with warnings.catch_warnings(record=True) as w:
            ...
            with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
                ...
                self.assertEqual(attributions.shape, xi.shape)
                np.testing.assert_almost_equal(
                    attributions, [[0.0, np.nan], [1.0, np.nan]], 10)
...
class TestShapleySamplingMethod(TestCase):
    ...
    def test_shapley_sampling(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            xi = np.array([[2, -2], [4, 2]])
            attributions = de.explain(
                'shapley_sampling', out, X, xi, samples=300)
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, [[0.0, -2.0], [1.0, 1.0]], 1)
    ...
    def test_shapley_sampling_batches(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            attributions = de.explain(
                'shapley_sampling', out, X, xi, samples=300, batch_size=5)
            self.assertEqual(attributions.shape, xi.shape)
            np.testing.assert_almost_equal(
                attributions, np.repeat([[0.0, -2.0], [1.0, 1.0]], 20, 0), 1)
    ...
    def test_shapley_sampling_batches_explainer_api(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            xi = np.array([[2, -2], [4, 2]]).repeat(20, 0)
            explainer = de.get_explainer(
                'shapley_sampling', out, X, samples=300)
    ...
    def test_shapley_sampling_dims(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            xi = np.array([[[-1, 4], [-2, 1]]])
            attributions = de.explain(
                'shapley_sampling', out, X, xi, samples=300, sampling_dims=[1])
    ...
    def test_shapley_targeting_equivalence(self):
        with DeepExplain(graph=tf.compat.v1.get_default_graph(), session=self.session) as de:
            ...
            self.assertEqual(out.shape[1], 2)
            np.random.seed(10)
            a1 = de.explain('shapley_sampling', out *
                            np.array([[1, 0]]), X, xi, samples=10)
            np.random.seed(10)
            b1 = de.explain('shapley_sampling', out *
                            np.array([[0, 1]]), X, xi, samples=10)
            np.random.seed(10)
            a2 = de.explain('shapley_sampling', out, X, xi,
                            ys=np.array([[1, 0]]), samples=10)
            np.random.seed(10)
            b2 = de.explain('shapley_sampling', out, X, xi,
                            ys=np.array([[0, 1]]), samples=10)
